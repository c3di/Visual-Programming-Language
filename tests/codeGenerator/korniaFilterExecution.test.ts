...
input_tensor1 = {
  ...
  'metadata': {
    ...
    'device': 'cpu'
      ...
      execTest: (inputs: any[], returnVar: any) => `import torch
from torch import Tensor
expected = K.filters.aussian_blur2d(input_tensor1['value'], ${inputs
        .slice(2)
        .join(', ')})
print(torch.equal(expected, ${returnVar}['value']));`,
...
${prepareInput}
${returnVar} = K.filters.aussian_blur2d(input_tensor1['value'], ${inputs
        .slice(2)
        .join(', ')})
${returnVar} = {
  'value': ${returnVar},
...
guidance_tensor = {
  'dataType': 'torch.tensor',
  'value': torch.rand(1, １, 5, 5, device = 'cpu'),
  'metadata': {
    'colorChannel': 'grayscale',
    'channelOrder': 'channelFirst',
    'isMiniBatched': True,
